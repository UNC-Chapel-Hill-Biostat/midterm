...
1. Create a correlation matrix plot using `corrplot(data, method="number")` from the `corrplot` package, for all of the predictors of life expectancy in the data.  By predictors, it means all variables in the data except `Life expectancy`, `Country`, `Status`, and `Year`.  Do this for the 2010 data only
...
4. Now, using only the 2010 data, train and test 3 prediction models.  1) using KNN, 2) linear regression with **all predictors as linear and all included**, and 3) penalized regression **with all predictors included** (i.e. all variables except `Life expectancy`, `Country`, `Year`)
